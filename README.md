...
**让 Codex 遇到难题时，自己多想一步。**

写代码时，你是否也需要反复判断：这次用 medium 够不够？卡住了要不要切 high？切完以后，还得提醒它接着做？

Adaptive Reasoning 是一个帮助 **Codex 按任务难度自动调整推理强度** 的轻量 Skill。启用后，它会在复杂问题或有依据的修复持续无进展时评估是否升档，保存进度，在同一任务的新一轮核验实际档位后继续开发，减少你手动切档和反复交代上下文的操作。

| 开发中的情况 | 它会如何处理 |
|---|---|
| 日常开发、普通修改 | 保持你选择的模型和初始强度 |
| 难题需要更深入的推理 | 评估后逐档提高强度，在同一任务接续 |
| 难题已验证解决，还有较长普通工作 | 满足冷却条件后恢复初始档位 |
| 希望必要时换用其他模型 | 仅按你明确授权的路线切换 |
